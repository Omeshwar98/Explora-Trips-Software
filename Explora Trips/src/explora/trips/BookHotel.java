    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == b1) {
            try {
                ...
                // Check how many times the user has booked this specific hotel
                String checkSql = "SELECT COUNT(*) FROM bookhotel WHERE username=? AND hotelname=?";
                PreparedStatement checkPs = conn.c.prepareStatement(checkSql);
                checkPs.setString(1, user);
                checkPs.setString(2, t1.getText()); // Hotel Name
                ResultSet rs = checkPs.executeQuery();

                if (rs.next()) {
                    int count = rs.getInt(1); // Get the number of bookings for this hotel
                    if (count >= 2) {
                        JOptionPane.showMessageDialog(null, "This Hotel is fully booked. Try another Hotel for Booking.");
                        return; // Stop further execution
                    }
                }

                // If bookings for this hotel are less than 2, allow a new booking
                String sql = "INSERT INTO bookhotel VALUES(?, ?, ?, ?, ?, ?, ?, ?)";
                PreparedStatement ps = conn.c.prepareStatement(sql);
                ps.setString(1, t1.getText());  // Hotel Name
                ps.setString(2, t2.getText());  // Customer Name
                ps.setString(3, t3.getText());  // Username
                ps.setString(4, t7.getText());  // Total Persons
                ps.setString(5, t8.getText());  // No. of Days
                ps.setString(6, c1.getSelectedItem()); // AC/Non-AC
                ps.setString(7, c2.getSelectedItem()); // Food Included
                ps.setString(8, l13.getText()); // Total Price
                ps.executeUpdate();
