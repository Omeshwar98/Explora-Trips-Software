...
public class HotelDetails extends JPanel implements ActionListener {
    ...
    HotelDetails() {
        ...
        bDelete = new JButton("Delete Hotel"); // Initialize delete button
    ...
    public void display() {
        try {
            ...
            if (rs.next()) {
                l3.setText(rs.getString(1)); // Hotel Name
                l4.setText(rs.getString(3)); // AC Cost
                l7.setText(rs.getString(2)); // City/State
                l8.setText(rs.getString(4)); // Food Cost
                l12.setText(rs.getString(5)); // Hotel Cost
        ...
        if (ae.getSource() == b1) {
            ...
            if (confirm == JOptionPane.YES_OPTION) {
                try {
                    ...
                    // Optionally, navigate back to the hotel panel after deletion
                    panel.setVisible(false);
                    panel1.setVisible(false);
                    HotelPanel hp = new HotelPanel(a); // Assuming admin is viewing
                    add(hp);
                    hp.setVisible(true);
                } catch (SQLException e) {
                    e.printStackTrace();
                    JOptionPane.showMessageDialog(this, "Error deleting hotel.");
                }
            }
        }
    }
